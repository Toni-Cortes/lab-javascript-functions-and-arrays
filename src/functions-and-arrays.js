...
function sum(arr) {
  ...
  arr.forEach(function (element) {
    if (typeof element === 'number') {
      ...
      throw new Error("Error, different data type");
    }
  });
...
function doesWordExist(words, wordToFind) {
  if (words.length === 0) return null;
  let isThereWord;
  if (words.includes(wordToFind)){isThereWord = true} else {isThereWord = false};
  return isThereWord;
}
...
function howManyTimes(words, wordToCount) {
  let wordCounter = 0;
  words.forEach(function(element){
    if (element === wordToCount){ wordCounter++}
  })
  return wordCounter;
 }
...
function greatestProduct(matrix) {
  ...
  for (i=0; i<matrix.length;i++){
    for (j=0; j<=matrix[i][j].length - 4; j++){
      let currentProduct = matrix[i][j]*matrix[i+1][j]*matrix[i][i+2]*matrix[i+3][j];
      if (currentProduct > maxProduct){
        maxProduct = currentProduct;
      }
    }
